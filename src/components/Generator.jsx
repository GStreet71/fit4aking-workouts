        ...
        <div className='flex flex-col gap-4'>
            <div className='flex items-center justify-center gap-2'>
                <p className='text-3xl sm:text-4xl md:text-5xl 
                    font-semibold text-yellow-400'>{ index }
                </p>
...
function Generator(props) {
    const { muscles, setMuscles, poison, setPoison,  goal, setGoal, updatedWorkout} = props
    ...
    return (
        ...
        {["Embrace", " Your", " Crown"]} >
                ...
                {Object.keys(WORKOUTS).map((type, typeIndex) => {
                    return (
                        <button 
                            ...
                            className= {
                            ` bg-slate-950 border duration-200
                            px-4 hover:border-blue-600 py-3 rounded-lg ` + 
                            (type === poison ? 'border-blue-600 rounded-lg' : 
            ...
            description = { 'Select the muscles judged for annihalation.' } />
            <div className="bg-slate-950 border border-solid 
                border-blue-400 rounded-lg flex flex-col">
                <button onClick={ toggleModal } 
                    className="relative p-3 flex items-center justify-center">
                    <p className='capitalize'>{muscles.length == 0 ? 'Select muscle groups' : muscles.join(" ")}</p>
                    <i className="absolute right-3 top-1/2 -translate-y-1/2 fa-solid 
                    fa-caret-down"></i>
                </button>
                { showModal && (
                    <div className='flex flex-col px-3 pb-3 '>
                        {(poison === 'individual' ?  WORKOUTS[poison] : Object.keys(WORKOUTS
                        [poison])).map((muscleGroup, muscleGroupIndex) => {
                            return (
                                <button onClick={() => {
                                    updateMuscles(muscleGroup)
                                }} key={muscleGroupIndex}
                                className={'hover:text-blue-400 duration-200 ' + (muscles.
                                includes(muscleGroup) ? 'text-blue-400' : ' ')}>
            ...
            <Header index = {'03'} title={'Becaome a Juggernaut'} 
